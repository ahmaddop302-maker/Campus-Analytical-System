...
1. **File Handler (M1):** Custom CSV/TXT parsing and writing logic.
2. **Student Operations (M2):** Add, search, update, and soft-delete students. Features manual substring matching for search-as-you-type.
3. **Course Operations (M3):** Enroll/drop courses, validate credit loads (max 21 hours), and check prerequisites.
4. **Attendance Module (M4):** Mark attendance, compute percentages, flag shortages (< 75%), and undo previous sessions.
